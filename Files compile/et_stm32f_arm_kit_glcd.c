...
unsigned char GLCD_SPI_Read_Write(unsigned char DataByte)    
{
  ...
  /* Send the byte */
  SPI_I2S_SendData(LCD_SPI, DataByte);

  /* Wait until a data is received */
  while (SPI_I2S_GetFlagStatus(LCD_SPI, SPI_I2S_FLAG_RXNE) == RESET);

  /* Get the received data */
  DataByte = SPI_I2S_ReceiveData(LCD_SPI);
...
void Initial_GLCD_Hor(void)
{
  ...
  //GLCD_Write_Data(0x2700);    																// Gate Scan Line For ET-TFT240320TP3.2 RevB 					
  GLCD_Write_Data(0xA700);																    // Gate Scan Line For ET-TFT240320TP3.2 RevC 
...
/********************************************************************************/ 
void Ellipse_Hor(unsigned int h,unsigned int k,long rx,long ry,char tick,long color,char fill)
{
  char bd;
  long   ax  =(rx*rx);
  long   by  =(ry*ry);
...
/*																					*/
/************************************************************************************/																						  
void Rectan_Hor(long x1,long y1,long x2,long y2,char tick,long color,char fill)
{
...
void Initial_GLCD_Ver(void)
{
  ...
  GLCD_Write_Data(0x0700);	 																// Line inversion 0x0700
  GLCD_Write_Command(0x03);    																// Configure Entry Mode
  GLCD_Write_Data(0x1230); 	 																// Color:RGB ,Incremen Hor.&Ver.address,Address update Hor.
  ...
  GLCD_Write_Command(0x29);
  GLCD_Write_Data(0x000F);      
  delay_ms(20);    																			//Delay for 40ms while the power supply stabilizes.
...
{	
  GLCD_Write_Command(0x20);  																// Command Set Adddress Hor(X)
  GLCD_Write_Data(Xadd);  																	// Sent X_address for CGRAM 		
  GLCD_Write_Command(0x21);  																// Command Set Address Ver(Y)
  GLCD_Write_Data(Yadd);  																	// Sent Y_address for CGRAM 	  			
  GLCD_Write_Command(0x22);  																// Command Write Data RGB
  GLCD_Write_Data(color);  																	// write data
}
...
void plot_mark_Ver(long x1,long y1,long x2,long y2,long color)
{
  ...
  // Plot Hor Line
  GLCD_Write_Command(0x20);  																//Command Set Adddress Hor(X)
  GLCD_Write_Data(x1);  																	//Sent X_address  CGRAM
	 		
  GLCD_Write_Command(0x21);  																//Command Set Address Ver(Y)
  GLCD_Write_Data(y1);  																	//Sent Y_address  CGRAM
  ...
  // Plot Ver Line
  GLCD_Write_Command(0x03);   																//Command Configure Entry Mode
  GLCD_Write_Data(0x1238); 	 																//Color:RGB ,Incremen Hor.&Ver.address,Address update Hor.

  GLCD_Write_Command(0x20);   																//Command Set Adddress Hor(X)
  GLCD_Write_Data(x2);   																	//Sent X_Address CGRAM 
	 		
  GLCD_Write_Command(0x21);   																//Command Set Address Ver(Y)
  GLCD_Write_Data(y2);	 																	//Sent Y_Address CGRAM 
...
void text_7x11_Ver(char row,long adx,long ady,long fg_clr,long bg_clr)
{
  ...
  for(m=0;m<14;m++)
  {
	...
	for(n=0;n<8;n++)		       															//Loop Sent data  1 byte(8bit)
	{
	  ...
	  {              
	    GLCD_Write_Command(0x20);  															//Command Set Adddress Hor(X)
        GLCD_Write_Data(ax);  																//Sent X_Address CGRAM	 		
        GLCD_Write_Command(0x21);  															//Command Set Address Ver(Y)
        GLCD_Write_Data(ay);  																//Sent Y_Address CGRAM
		GLCD_Write_Command(0x22);  															//Command Write data 
        GLCD_Write_Data(fg_clr); 
	  }
	  else						   															//if data bit7 = 0 ,Plot Color area back ground Charecter
...
/*            fill        = 0: No fill color in Ellipes,1:Fill color in Ellipes */
/********************************************************************************/ 
void Ellipse_Ver(unsigned int h,unsigned int k,long rx,long ry,char tick,long color,char fill)
{
  char bd;
  long   ax  =(rx*rx);
  long   by  =(ry*ry);
...
/*																					*/
/************************************************************************************/																						  
void Rectan_Ver(long x1,long y1,long x2,long y2,char tick,long color,char fill)
{
  long xmin,xmax,ymin,ymax,i;
  char tk = tick-1;
  ...
  {
    Line_Ver(x1,y1-tk,x2,y1-tk,tick,color);  													//Line Ho.r Top
	Line_Ver(x1,y2,x2,y2,tick,color);  															//Line Hor. Botton
	Line_Ver(x1,y1,x1,y2,tick,color);  															//Line Ver. Left
	Line_Ver(x2-tk,y1,x2-tk,y2,tick,color);  													//Line Ver. Right
  }      		 
}
...
void plot_tab_Ver(unsigned long color)
{
  ...
  { 
    GLCD_Write_Command(0x20);  																//Command Set Adddress Hor(X)
    GLCD_Write_Data(200);  																	//Sent X_address for CGRAM 
	GLCD_Write_Command(0x21);  																//Command Set Address Ver(Y)
    GLCD_Write_Data(309+ry);  																//Sent Y_address for CGRAM 			
    GLCD_Write_Command(0x22);  																//Command Write Data RGB
...
/*																	     */
/*************************************************************************/
void button_build_Ver(long px,long py,long width,long high,long c_bt,unsigned char *tx,char num_tx,long c_tx)
{
  long x1,y1,x2,y2,ay,tpx;
  char tab,n;
  long cbt1=0,cbt2=0,sub=0;
...
void plot_picture_Ver(const char *bmp,long xad,long yad,long width,long high)        
{
  ...
  {      		
    GLCD_Write_Command(0x21);  																//Command Set Address Ver(Y)
    GLCD_Write_Data(yad1);  																//Sent Y_Address CGRAM		 		  

	for(wid=0;wid<width;wid++)	           													//number X-pixel(Width) of Picture  
	{
	  GLCD_Write_Command(0x20);  															//Command Set Adddress Hor(X)
      GLCD_Write_Data(xad1);  																//Sent X_Address CGRAM	
